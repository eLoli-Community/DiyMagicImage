...
class Base
{
    ...
    {
        $path=self::get_base(app()->resourcePath(),$type);
        if($file){
            $path=self::get_base($path,$file);
        }
        return $path;
    }
    ...
    {
        return self::get_base(app()->basePath('app/GD/app'),$file);
    }
    ...
    public static function scanDir($path){
        $path=self::formt_dir($path);
        $files=[];
        foreach(File::allFiles($path) as $file){
            $files[substr($file->getPathname(),strlen($path))]=$file->getPathname();
        }
        return $files;
    }

    public static function get_base($base,$file){
        $base=self::formt_dir($base);
        $file = self::formt_dir($base.'/'.$file);
        if(self::start_with($file,$base)){
            return substr($file,0,strlen($file)-1);
        }else{
    ...
    public static function start_with($str,$start)
    {
